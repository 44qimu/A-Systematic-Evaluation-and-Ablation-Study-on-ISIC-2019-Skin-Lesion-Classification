...
class ISICDataset(Dataset):
    ...
    def __init__(self, mdlParams, indSet):
        ...
        # ============================================================
        # ✅ FIX (minimal, keeps logic): ensure indices used for numpy
        # indexing are integer/bool, otherwise numpy will throw:
        # "arrays used as indices must be of integer (or boolean) type"
        # ============================================================
        self.indices = np.asarray(self.indices)
        if self.indices.dtype.kind not in ("i", "b"):
            # guard against NaN / non-integer values
            if np.isnan(self.indices).any():
                raise ValueError(f"[ISICDataset] {indSet} has NaN indices, cannot index arrays. sample={self.indices[:20]}")
            if not np.all(np.equal(self.indices, np.round(self.indices))):
                raise ValueError(f"[ISICDataset] {indSet} has non-integer indices. sample={self.indices[:20]}")
            self.indices = self.indices.astype(np.int64)
        # keep mdlParams consistent for later uses in this class
        mdlParams[indSet] = self.indices
        # ============================================================
        ...
        if self.balancing == 3 and indSet == 'trainInd':
            ...
            if 'testInd' in mdlParams:
                print("Intersect val",np.intersect1d(indices,mdlParams['valInd']),
                      "Intersect Testind",np.intersect1d(indices,mdlParams['testInd']))
            else:
                print("Intersect val",np.intersect1d(indices,mdlParams['valInd']))
            ...
            if mdlParams.get('eval_flipping',0) > 1:
                ...
                inds_rep = np.asarray(inds_rep, dtype=np.int64)  
                ...
                inds_rep = np.asarray(inds_rep, dtype=np.int64)  # ✅ FIX: robust
                self.labels = mdlParams['labels_array'][inds_rep,:]
                # meta
                if mdlParams.get('meta_features',None) is not None:
                    self.meta_data = mdlParams['meta_array'][inds_rep,:]
            ...
            if self.multiCropEval == 0:
                if self.only_downsmaple:
                    ...
                    self.cropping = transforms.Compose([
                        transforms.CenterCrop(np.int32(self.input_size[0] * 1.5)),
                ...
                if self.mdlParams.get('deterministic_eval', False):
                    ...
                    self.cropPositions = np.tile(
                        self.cropPositions, (mdlParams[indSet].shape[0], 1)
                    )
                    ...
                    self.cropping = transforms.RandomResizedCrop(
                        self.input_size[0],
                        scale=(mdlParams.get('scale_min', 0.08), 1.0),
                    )
                    # Complete labels array, only for current indSet, repeat for multiordercrop
                    inds_rep = np.repeat(mdlParams[indSet], mdlParams['multiCropEval'])
                    inds_rep = np.asarray(inds_rep, dtype=np.int64)  
            ...
            self.norm = transforms.Normalize(
                np.float32(self.mdlParams['setMean']),
        ...
        if self.preload:
            ...
            for i in range(len(self.im_paths)):
                self.im_list.append(Image.open(self.im_paths[i]).convert("RGB"))
    ...
    def __getitem__(self, idx):
        ...
        if self.preload:
            ...
            x = Image.open(self.im_paths[idx]).convert("RGB")
        ...
        if self.orderedCrop and (self.indSet == 'valInd' or self.indSet == 'testInd' or self.train_eval_state == 'eval'):
            ...
            if self.mdlParams.get('deterministic_eval',False):
                ...
                if crop == 0:
                    ...
                    x = transforms.functional.crop(
                        x,
                        int(self.mdlParams['offset_crop']*x.size[0]),
                        int(self.mdlParams['offset_crop']*x.size[1]),
                    ...
                    x = transforms.functional.crop(
                        x,
                        int(self.mdlParams['offset_crop']*x.size[0]),
                        int((1.0-self.mdlParams['offset_crop'])*x.size[1]-self.input_size[1]),
                        self.input_size[0], self.input_size[1]
                    )
                elif crop == 3:
                    x = transforms.functional.crop(
                        x,
                        int((1.0-self.mdlParams['offset_crop'])*x.size[0]-self.input_size[0]),
                        int(self.mdlParams['offset_crop']*x.size[1]),
                        self.input_size[0], self.input_size[1]
                    )
                elif crop == 4:
        ...
        if self.mdlParams.get('eval_flipping',0) > 1:
            if self.mdlParams.get('meta_features',None) is not None:
                return (x, x_meta), y, idx, self.flipPositions[idx]
            else:
...
def getErrClassification_mgpu(mdlParams, indices, modelVars, exclude_class=None):
    ...
    if mdlParams.get('eval_flipping',0) > 1 and mdlParams.get('multiCropEval',0) > 0:
        ...
        if mdlParams.get('model_type_cnn') is not None and mdlParams['numRandValSeq'] > 0:
            ...
            for i, (inputs, labels, inds) in enumerate(modelVars['dataloader_'+indices]):
                ...
                predictions_mc[i,:,:] = preds.detach().cpu().numpy().T
            ...
            for i, (inputs, labels, indices_) in enumerate(modelVars['dataloader_'+indices]):
                if mdlParams.get('meta_features',None) is not None:
                    inputs[0] = inputs[0].to(modelVars['device'])
                    inputs[1] = inputs[1].to(modelVars['device'])
                else:
    ...
    den = conf.sum(axis=1)
    wacc = np.divide(conf.diagonal(), den, out=np.zeros_like(den, dtype=float), where=den!=0)
    ...
    if num_classes > 2:
        for k in range(num_classes):
            ...
            sensitivity[k] = conf[k,k]/row_sum if row_sum != 0 else 0.0
            ...
            specificity[k] = true_negative/denom if denom != 0 else 0.0

        f1 = f1_score(y_true, y_pred, average='weighted')
    else:
        cm2 = confusion_matrix(y_true, y_pred, labels=np.arange(num_classes))
        if cm2.size == 4:
    ...
    for i in range(num_classes):
        try:
            ...
            fpr[i], tpr[i] = np.array([0.0, 1.0]), np.array([0.0, 1.0])
            roc_auc[i] = np.nan

    if (mdlParams.get('eval_flipping',0) > 1 and mdlParams.get('multiCropEval',0) > 0) or (mdlParams.get('multiCropEval',0) > 0):
        loss_mean = float(np.mean(loss_mc))
    else:
        loss_mean = float(np.mean(loss_all))

    return loss_mean, acc, sensitivity, specificity, conf, f1, roc_auc, wacc, predictions, targets, predictions_mc
