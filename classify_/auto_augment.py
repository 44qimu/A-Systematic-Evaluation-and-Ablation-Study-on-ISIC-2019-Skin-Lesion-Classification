...
def cutout(org_img, magnitude=None):
    ...
    Notes
    -----
    - Accepts a PIL image (or numpy array) and returns a PIL image.
    - The mask size follows the original implementation: scaled by image height.
    """
    # Convert to numpy array we can modify safely
    img = np.array(org_img).copy()
    ...
    if magnitude is None:
        ...
        mask_size = int(round(h * random.uniform(magnitudes[magnitude], magnitudes[magnitude + 1])))

    # Keep mask size in a valid range
    mask_size = max(1, min(mask_size, h, w))

    # Allow starting slightly outside the image, then clip
    top_low = -mask_size // 2
    top_high = h - mask_size + 1
    left_low = -mask_size // 2
    left_high = w - mask_size + 1

    top = 0 if top_high <= top_low else np.random.randint(top_low, top_high)
    left = 0 if left_high <= left_low else np.random.randint(left_low, left_high)

    top = 0 if top < 0 else top
    left = 0 if left < 0 else left

    bottom = min(h, top + mask_size)
    right = min(w, left + mask_size)
